...
class MarsRoverPhotoRepoTest {
    ...
    fun `should emit success when service and dao return valid data`() =
        runTest(coroutineRule.testDispatcher) {
            ...
            } returns flowOf(listOf(2))  // Considerando que a primeira foto está salva id = 2


            //When
            val marsRoverPhotoRepo = MarsRoverPhotoRepo(
                marsRoverPhotoService,
                marsRoverSavedPhotoDao
            )

            val result = marsRoverPhotoRepo.getMarsRoverPhoto(
                "perseverance",
                "0"
            ).toList()
    ...
    fun `should emit error when service throw exception`() = runTest(coroutineRule.testDispatcher) {
        ...
        val result = marsRoverPhotoRepo
            .getMarsRoverPhoto("perseverance", "0").toList()
    ...
    @Test
    fun `should call insert when save photo is called `() = runTest(coroutineRule.testDispatcher) {
        ...
        //Then
        coEvery { marsRoverSavedPhotoDao.insert(marsRoverSavedLocalModel) }
    ...
    @Test
    fun `should call delete when remove photo is called `() =
        runTest(coroutineRule.testDispatcher) {
